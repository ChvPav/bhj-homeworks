...
elementHasHint.forEach(element => element.addEventListener('click', (ev) => {
    ...
    ev.target.insertAdjacentHTML("afterEnd", `<div class="tooltip tooltip_active" style="position:absolute; top:${location.bottom + 5}px; left:${location.left}px">${ev.target.title}</div>`);
